...
const { data: page } = await useAsyncData(route.path, () => {
  return queryCollection('skills').path(route.path).first()
})
...
useSeoMeta({
  title: page.value?.title || 'Skill',
  description: `Skill: ${page.value?.title} — ${page.value?.category}`
})
...
async function deleteItem() {
  ...
  const slug = page.value.path.replace('/skills/', '')
  ...
  try {
    ...
    toast.add({
      title: 'خطأ',
      description: err.data?.message || 'فشل في حذف الـ Skill',
      color: 'error',
