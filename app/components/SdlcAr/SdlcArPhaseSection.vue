<template>
  <section :id="phase.id" class="py-20 relative" @mousemove="onMouseMove">
    <!-- Phase accent glow background -->
    <div class="phase-bg-glow" :style="`--phase-color: var(--color-${phase.color}-500, #06b6d4)`" />
    ...
    <UContainer>
      ...
      >
        ...
        <div>
          <div class="text-4xl font-black sdlc-display" :class="`text-${phase.color}-400`">
            {{ phase.title }}
          </div>
          <div class="text-base text-neutral-500 mt-1">{{ phase.tagline }}</div>
        </div>
      </div>
      ...
      <div class="grid md:grid-cols-3 gap-5 mb-6">
        ...
        >
          <div class="badge badge-a mb-4">⚡ الطبقة أ · نظرة سريعة</div>
          <h4 class="text-base font-bold mb-3 sdlc-display text-neutral-100">{{ phase.layerA.title }}</h4>
          <p class="text-sm text-neutral-400 leading-relaxed mb-5">{{ phase.layerA.body }}</p>
          <div class="output-box" :style="`--pc: var(--color-${phase.color}-500, #06b6d4)`">
            <div class="output-label" :class="`text-${phase.color}-400`">المخرج الرئيسي</div>
            <div class="text-sm text-neutral-200 font-medium leading-relaxed">{{ phase.layerA.output }}</div>
          </div>
        </div>
        ...
        <div
          ...
          :style="`--pc: var(--color-${phase.color}-500, #06b6d4)`"
        >
          <div class="badge badge-b mb-4">📋 الطبقة ب · منظّمة</div>
          <h4 class="text-base font-bold mb-4 sdlc-display text-neutral-100">{{ phase.layerB.title }}</h4>
          <ul class="layer-b-list">
            <li
              ...
              class="layer-b-item"
            >
              <span class="item-dot" :class="`bg-${phase.color}-500`" />
        ...
        <div
          class="layer-c-card"
        >
          <div class="badge badge-c mb-4">🧠 الطبقة ج · ملاحظات المعماري</div>
          <h4 class="text-base font-bold mb-4 sdlc-display text-neutral-100">{{ phase.layerC.title }}</h4>
          <ul class="space-y-3">
            <li
      ...
      <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div
          ...
          class="meta-item"
        >
          <div class="meta-label">{{ metaItem.label }}</div>
          <div class="meta-value">{{ metaItem.value }}</div>
        </div>
      </div>
    </UContainer>
  </section>
...
const metaItems = computed(() => [
  { label: 'الأدوار', value: props.phase.meta.roles },
  { label: 'السؤال الجوهري', value: props.phase.meta.question },
  { label: 'محفّز المرحلة التالية', value: props.phase.meta.trigger },
  { label: 'الخطر عند التخطي', value: props.phase.meta.risk },
])
